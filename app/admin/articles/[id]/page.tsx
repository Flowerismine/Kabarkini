...
export default function NewArticlePage() {
  ...
  // ── Simpan artikel ─────────────────────────────────────────────────────
  // TODO (Lapisan 1.3): Ganti mock POST ini dengan Supabase insert
  // setelah data layer selesai dimigrasikan dari mock-data.tsx
  const handleSave = async () => {
    ...
    try {
      ...
      // Arahkan ke halaman edit artikel yang baru dibuat
      setTimeout(() => {
        router.push(data?.id ? `/admin/articles/${data.id}` : '/admin/articles')
      }, 1000)
    } catch (err) {
    ...
    <div className="min-h-screen">
      ...
      <header className="bg-white border-b border-border px-6 py-3 flex items-center justify-between gap-4 sticky top-0 z-10 shadow-sm">
        ...
        <div className="flex items-center gap-2 shrink-0">
          {/* Error inline */}
          {saveState === 'error' && (
            <span className="text-xs text-red-600 flex items-center gap-1 max-w-xs truncate">
          ...
          <button
            ...
            className="flex items-center gap-1.5 text-xs bg-[var(--navy)] text-white px-4 py-1.5 rounded-lg hover:bg-[var(--navy-light)] transition-colors disabled:opacity-60 disabled:cursor-not-allowed"
          >
            {saveState === 'saving' ? (
                ...
                <div className="bg-white rounded-xl border border-border p-5 space-y-4">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      placeholder="Contoh: kebijakan ekonomi 2025"
                      className="w-full text-sm border border-border rounded-lg px-3 py-2 outline-none focus:border-[var(--navy)] transition-colors"
                    />
                  </div>
                </div>

                {/* SERP Preview — tampil jika ada konten */}
                  ...
                  <div className="bg-white rounded-xl border border-border p-5">
                    ...
                    <div className="border border-border rounded-lg p-4 bg-white">
                      ...
                      <p className="text-[#545454] text-sm mt-1 line-clamp-2">
                        {form.metaDesc || form.excerpt || 'Meta description akan muncul di sini.'}
                      </p>
                    </div>
                  </div>
                )}
          ...
          <aside className="xl:col-span-1 space-y-4">
            ...
            <div className="bg-white rounded-xl border border-border p-5">
              ...
              <button
                onClick={handleSave}
                disabled={saveState === 'saving' || saveState === 'saved'}
                className="w-full mt-3 bg-[var(--navy)] text-white text-sm font-semibold py-2.5 rounded-lg hover:bg-[var(--navy-light)] transition-colors flex items-center justify-center gap-2 disabled:opacity-60 disabled:cursor-not-allowed"
              >
                {saveState === 'saving' ? (
            ...
            <div className="bg-white rounded-xl border border-border p-5">
              ...
              <div className="space-y-2.5">
                {(
                  [
                    { label: 'Breaking News', key: 'isBreaking' },
                    { label: 'Artikel Unggulan', key: 'isFeatured' },
                    { label: 'Sedang Trending', key: 'isTrending' },
                  ] as { label: string; key: keyof NewArticleForm }[]
                ).map(({ label, key }) => (
                  <label key={key} className="flex items-center justify-between cursor-pointer">
                    <span className="text-sm text-foreground">{label}</span>
                    <button
                      onClick={() => set(key, !form[key] as NewArticleForm[typeof key])}
                      className={cn(
